...
public:
	...
	struct Options
	{
		Options()
		{
			...
			m_eps = -1; // not used
			m_iterMax = 1000000;
			m_printIter = 100;     // After 10 iterations start printing the lower bound
			m_printMinIter = 100; // and the energy every 5 iterations.
		}
	...
	int Minimize_TRW_S(bool IsUseGPU,Options& options, REAL& lowerBound, REAL& energy, REAL* min_marginals = NULL);
...
private:
    ...
    void doCudaProcess(Node*,Node*,REAL&,bool,REAL*,REAL**);
    void InitializeCudaProcess(Node*,Node*);
    void MRFEnergy<T>::ReleaseCudaProcess();
    ...
    struct CUDA{
        ...
        REAL* e_forward;
        REAL* e_backward;
        REAL* EdgeMessage_forward;
        REAL* EdgeMessage_backward;
        REAL* buf;
        REAL* this_data;
        REAL* gamma_forward;
        REAL* gamma_backward;
        int* forward_backward;
        int* backward_forward;
    };
    CUDA cu;
};
...
template <class T> void MRFEnergy<T>::CompleteGraphConstruction()
{
	...
	mexPrintf("Completing graph construction... ");
...
template <class T> int MRFEnergy<T>::Minimize_TRW_S(bool IsUseGPU,Options& options, REAL& lowerBound, REAL& energy, REAL* min_marginals)
{
	Node* i;
	Node* j;
	MRFEdge* e;
    ...
    if((cudaStatus==cudaSuccess)&&IsUseGPU){
        InitializeCudaProcess(m_nodeFirst,m_nodeLast);
	...
	for (iter=1; ; iter++)
	{
        ...
        if(cudaStatus!=cudaSuccess||(!IsUseGPU)){
            for (i=m_nodeFirst; i; i=i->m_next)
            {
                ...
                //mexPrintf("aaa  %lf %lf %lf %lf %lf\n",(i->m_D).GetArrayHead()[1],(i->m_D).GetArrayHead()[3],(i->m_D).GetArrayHead()[5],(i->m_D).GetArrayHead()[7],(i->m_D).GetArrayHead()[9]); ///////////////////////////////////////
                
                /*e=i->m_firstForward;
                if(e)
                    mexPrintf("bbb  %lf %lf %lf %lf %lf\n",e->m_message.GetMessagePtr()[1],e->m_message.GetMessagePtr()[3],e->m_message.GetMessagePtr()[5],e->m_message.GetMessagePtr()[7],e->m_message.GetMessagePtr()[9]); ///////////////////////////////////////
                */
                ...
                */
                ...
                //mexPrintf("a  %lf %lf %lf %lf %lf\n",Di->GetArrayHead()[1],Di->GetArrayHead()[3],Di->GetArrayHead()[5],Di->GetArrayHead()[7],Di->GetArrayHead()[9]); ///////////////////////////////////////
            ...
            for (i=m_nodeLast; i; i=i->m_prev)
            {
                ...
                //mexPrintf("a_back  %lf %lf %lf %lf %lf\n",Di->GetArrayHead()[1],Di->GetArrayHead()[3],Di->GetArrayHead()[5],Di->GetArrayHead()[7],Di->GetArrayHead()[9]); ///////////////////////////////////////
            ...
            doCudaProcess(m_nodeFirst,m_nodeLast,lowerBound,lastIter,min_marginals,&min_marginals_ptr);
        }
		...
		)
		{
			...
			mexPrintf("iter %d: lower bound = %f, energy = %f\n", iter,lowerBound,energy);
		}
    ...
    if((cudaStatus==cudaSuccess)&&IsUseGPU){
        mexPrintf("Cuda begins to release\n");
        ReleaseCudaProcess();
    }
	return iter;
}
...
template <class T> void MRFEnergy<T>::SetAutomaticOrdering()
{
	...
	mexPrintf("Setting automatic ordering... ");
